...
static unsigned char* baseOHTKeyPointer(BaseOHT* const table, const size_t idx) {
    assert(table);

    return table->buckets_data + idx * table->bucket_alloc_size;
}

static unsigned char* baseOHTValuePointer(BaseOHT* const table, const size_t idx) {
    assert(table);

    return baseOHTKeyPointer(table, idx) + table->key_alloc_size;
}

static unsigned char* baseOHTFlagPointer(BaseOHT* const table, const size_t idx) {
    assert(table);
...
static void baseOHTSetFlag(BaseOHT* const table, const size_t idx, const BaseOHTState new_state) {
    assert(table);

    *baseOHTFlagPointer(table, idx) = new_state;
}

static size_t baseOHTIdx(BaseOHT const* const table, const uint64_t hash, const size_t i) {
    assert(table);
...
static size_t baseOHTFindInUseIdx(BaseOHT* const table, void const* const key) {
    ...
    for (size_t i = 0; i < table->size + 1; i++) {
        ...
        if (current_state == OPEN_HASH_TABLE_IN_USE && table->compare_func(current_key_ptr, key)) {
            return idx;
        }
    }
...
static size_t baseOHTFindInsertIdx(BaseOHT* const table, void const* const key) {
    assert(table && key);
...
static IdxAndState baseOHTFindInsertOrInUseIdx(BaseOHT* const table, void const* const key) {
    assert(table && key);
...
static void* baseOHTInsertAt(BaseOHT* const table, const size_t insert_idx, void const* const key, void const* const value) {
...
BaseOHT* baseOHTInit(const size_t key_size, const size_t key_align, const size_t value_size, const size_t value_align, const BaseHashFunc hash_func, const BaseCompareFunc compare_func) {
    ...
    table->bucket_alloc_size = align(table->key_alloc_size + table->value_size + 1, max(table->key_align, table->value_align));
...
static bool baseOHTResize(BaseOHT* const table, size_t new_capacity) {
    ...
    new_table.buckets_data = calloc(new_table.capacity, new_table.bucket_alloc_size);
...
void* baseOHTInsert(BaseOHT* const table, void const* const key, void const* const value) {
    assert(table && key && value);
...
void* baseOHTFindOrInsert(BaseOHT* const table, void const* const key, void const* const value) {
    assert(table && key && value);
