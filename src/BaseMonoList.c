...
MonoListNode *monoListPopBack(
    MonoList *ml) { // not sure if the old end should be deleted here
  assert(ml && "the list is not defined");
...
size_t monoListItemSize(MonoList *ml) { return ml->key_size; }

size_t monoListSize(MonoList *ml) { return ml->length; }

MonoListNode *monoListNodeNext(MonoListNode *node) { return node->next; }
