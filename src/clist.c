...
void cremove_from_end(struct cycle_t* start);

struct cycle_t* cinsert_in_front(struct cycle_t* start, const void* src) // need optimization
{
	assert(start);
	while(cur_size + sizeof((char*)src) > MAX_CASH_SZ) {
		cremove_from_end(start);
	}
...
void cremove_from_end(struct cycle_t* start)//done
{
	assert(start);
	start = start->prev;
...
struct cycle_t* cmove_forward(struct cycle_t* cur, struct cycle_t* start)//done
{
	assert(start);
	assert(cur);
...
void cshow_list(struct cycle_t* start)// for int only, not tested yet
{
	printf("%d ", *(start->data));
	if(start->next != NULL) {
...
void cfree_list(struct cycle_t* cur, struct cycle_t* start)//done
{
	assert(cur);
